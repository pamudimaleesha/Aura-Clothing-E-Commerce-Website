...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['feedback_id'], $_POST['feedback_status'])) {
    ...
    $feedback_status = in_array($_POST['feedback_status'], ['inactive', 'active'])
        ? $_POST['feedback_status']
        : 'inactive';

    $stmt = $conn->prepare("UPDATE feedback SET status = ? WHERE f_id = ?");
    
    if ($stmt->execute([$feedback_status, $feedback_id])) {
...
<head>
    ...
    <title>Feedback Management - Auraclothing</title>
    <!-- Bootstrap CSS -->
